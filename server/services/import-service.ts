...
export class ImportService {
  ...
  async importPart(partData: InsertPart, idVehiculoOriginal?: number): Promise<Part> {
    try {
      ...
      // Verificar si el precio es 0 o undefined
      const price = Number(sanitizedPartData.precio);
      const hasValidPrice = !isNaN(price) && price > 0;

      if (!hasValidPrice) {
        console.log(`Pieza con precio ${price} será desactivada: refLocal=${sanitizedPartData.refLocal}`);
      ...
      if (existingParts.length > 0) {
        ...
        console.log(`Pieza actualizada: ID=${updatedPart?.id}, refLocal=${updatedPart?.refLocal}, precio=${updatedPart?.precio}`);
        part = updatedPart!;
      } else {
        // Crear nueva pieza (inicialmente desactivada)
        sanitizedPartData.activo = false;
        part = await storage.createPart(sanitizedPartData);
        console.log(`Nueva pieza creada: ID=${part.id}, refLocal=${part.refLocal}, precio=${part.precio}`);
      }

      // Solo permitir activación si tiene precio válido y vehículos asociados
      isActive = hasValidPrice && isActive;

      // Si el precio no es válido, forzar estado inactivo
      if (!hasValidPrice) {
        isActive = false;
      }
  ...
  private async createPendingRelation(partId: number, idVehiculoOriginal: number): Promise<void> {
    try {
      // ID del vehículo dummy que creamos para las relaciones pendientes
      const DUMMY_VEHICLE_ID = 50956;

      // Usar el vehículo dummy para relaciones pendientes
      const pendingRelation: InsertVehiclePart = {
        vehicleId: DUMMY_VEHICLE_ID, // Vehículo dummy especial para relaciones pendientes
        partId,
        idVehiculoOriginal
      };
  ...
  async updateAllPartsActivationStatus(): Promise<{ activated: number, deactivated: number }> {
    try {
      ...
      for (const part of allParts) {
        ...
        const validRelations = relations.filter(rel => rel.vehicleId > 0);
        const shouldBeActive = validRelations.length > 0;
        ...
        if (part.activo !== shouldBeActive) {
          ...
          if (shouldBeActive) {
            console.log(`Pieza ID=${part.id} activada porque tiene ${validRelations.length} relaciones válidas con vehículos`);
            activated++;
          } else {
