...
import { importHistory, vehicles, parts, vehicleParts } from './shared/schema';
...
async function testScheduledImportAssociations() {
  ...
  try {
    ...
    if (lastImport.length > 0) {
      const importRecord = lastImport[0];
      console.log(`\n📅 Última importación encontrada:`);
      console.log(`   ID: ${importRecord.id}`);
      console.log(`   Tipo: ${importRecord.type}`);
      console.log(`   Estado: ${importRecord.status}`);
      console.log(`   Fecha: ${importRecord.createdAt}`);
      console.log(`   Procesados: ${importRecord.processedItems}`);
      console.log(`   Nuevos: ${importRecord.newItems}`);
      console.log(`   Actualizados: ${importRecord.updatedItems}`);
    }
