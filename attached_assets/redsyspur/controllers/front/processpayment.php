...
class RedsyspurProcessPaymentModuleFrontController extends ModuleFrontController {
	...
	private function businessLogic() {
		...
		if ($resultCode == RESTConstants::$RESP_LITERAL_OK) {
			...
			$returnURL_OK = $Linkobj->getPageLink('order-confirmation') . '?id_cart='.$cart->id.'&id_module='.$redsys->id.'&id_order='.Order::getOrderByCartId($cart->id).'&key='.$customer->secure_key;
			$response ["url"] = $urlOK ? $urlOK : $returnURL_OK;
		...
		escribirLog("DEBUG", $idLog, "Se redirgirá al cliente a " . $response ["url"]);
